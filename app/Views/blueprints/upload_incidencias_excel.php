...
const tiposInsectos = [
    ...
    'Otros',
    // Organismos especiales para trampas específicas
    'Pulga',
    'Chinche',
    // Organismos para Trampa de Feromona Gorgojo
    'Sithopilus sp.',
    'Lasioderma sp.',
    'Tribolium sp.',
    'Stegobium sp.',
    ...
    'Tenebroides sp.',
    'Otro (gorgojo)',
    // Organismos para Trampa de Feromona Palomilla
    'Plodia',
    'Ephestia',
    'Sitotroga',
    'Otra (palomilla)'
...
const mapeoTiposInsectos = {
    ...
    // Organismos especiales para trampas específicas
    'Pulga': { tipoPlaga: 'pulga', tipoInsecto: 'Rastrero' },
    'Chinche': { tipoPlaga: 'chinche', tipoInsecto: 'Rastrero' },
    // Organismos para Trampa de Feromona Gorgojo
    'Sithopilus sp.': { tipoPlaga: 'sithopilus', tipoInsecto: 'Rastrero' },
    'Lasioderma sp.': { tipoPlaga: 'lasioderma', tipoInsecto: 'Rastrero' },
    'Tribolium sp.': { tipoPlaga: 'tribolium', tipoInsecto: 'Rastrero' },
    'Stegobium sp.': { tipoPlaga: 'stegobium', tipoInsecto: 'Rastrero' },
    'Rizopertha sp.': { tipoPlaga: 'rizopertha', tipoInsecto: 'Rastrero' },
    'Orizaephilus sp.': { tipoPlaga: 'orizaephilus', tipoInsecto: 'Rastrero' },
    'Trogoderma sp.': { tipoPlaga: 'trogoderma', tipoInsecto: 'Rastrero' },
    'Dermestes sp.': { tipoPlaga: 'dermestes', tipoInsecto: 'Rastrero' },
    'Tenebroides sp.': { tipoPlaga: 'tenebroides', tipoInsecto: 'Rastrero' },
    'Otro (gorgojo)': { tipoPlaga: 'otro_gorgojo', tipoInsecto: 'Rastrero' },
    // Organismos para Trampa de Feromona Palomilla
    'Plodia': { tipoPlaga: 'plodia', tipoInsecto: 'Volador' },
    'Ephestia': { tipoPlaga: 'ephestia', tipoInsecto: 'Volador' },
    'Sitotroga': { tipoPlaga: 'sitotroga', tipoInsecto: 'Volador' },
    'Otra (palomilla)': { tipoPlaga: 'otra_palomilla', tipoInsecto: 'Volador' }
};

// Mapeo de tipos de trampa a tipos de plaga permitidos
const tiposPlagaPorTrampa = {
    'edc_quimicas': ['Roedor'],
    'edc_adhesivas': ['Roedor', 'Arañas', 'Cucaracha Americana', 'Cucaracha Alemana', 'Escarabajo', 'Gorgojos', 'Tijerilla', 'Otros'],
    ...
    'feromona_gorgojo': ['Sithopilus sp.', 'Lasioderma sp.', 'Tribolium sp.', 'Stegobium sp.', 'Rizopertha sp.', 'Orizaephilus sp.', 'Trogoderma sp.', 'Dermestes sp.', 'Tenebroides sp.', 'Otro (gorgojo)'],
    'feromona_palomilla': ['Plodia', 'Ephestia', 'Sitotroga', 'Otra (palomilla)'],
    'equipo_sonico': [], // No captura organismos, solo tiene estado de funcionamiento
    'globo_terror': [], // No captura organismos, solo tiene condición de equipo
    'atrayente_pulgas': [], // Solo captura SI/NO y cantidad
    'atrayente_chinches': [] // Solo captura SI/NO
};
...
const mapeoTipoPlagaATipoInsecto = {
    ...
    'stegobium': 'Rastrero',
    'rizopertha': 'Rastrero',
    'orizaephilus': 'Rastrero',
    'trogoderma': 'Rastrero',
    'dermestes': 'Rastrero',
    'tenebroides': 'Rastrero',
    'otro_gorgojo': 'Rastrero',
    // Organismos para Trampa de Feromona Palomilla
    'plodia': 'Volador',
    'ephestia': 'Volador',
    'sitotroga': 'Volador',
    'otra_palomilla': 'Volador'
};
...
let estadosInspectorGlobal = {}; // Objeto para almacenar el estado del checkbox y valor del inspector global por semana
    ...
    // Función para ordenar trampas por números en el nombre (si contiene números)
    // Si no contiene números, mantiene el orden original
    function ordenarTrampasPorNumero(trampas) {
        // Crear una copia del array para no mutar el original
        const trampasCopia = Array.isArray(trampas) ? [...trampas] : [];
        
        return trampasCopia.sort((a, b) => {
            // Validar que a y b existan
            if (!a || !b) {
                return 0;
            }
            
            const nombreA = (a.nombre || a.id_trampa || (a.id ? 'T' + a.id : '')) || '';
            const nombreB = (b.nombre || b.id_trampa || (b.id ? 'T' + b.id : '')) || '';
            
            // Convertir a string para asegurar que podemos trabajar con él
            const strA = String(nombreA);
            const strB = String(nombreB);
            
            // Extraer números del nombre A
            const matchesA = strA.match(/\d+/g);
            const numerosA = matchesA ? matchesA.map(n => parseInt(n, 10)).filter(n => !isNaN(n)) : [];
            
            // Extraer números del nombre B
            const matchesB = strB.match(/\d+/g);
            const numerosB = matchesB ? matchesB.map(n => parseInt(n, 10)).filter(n => !isNaN(n)) : [];
            
            // Si ambos tienen números, comparar por el primer número
            if (numerosA.length > 0 && numerosB.length > 0) {
                if (numerosA[0] !== numerosB[0]) {
                    return numerosA[0] - numerosB[0];
                }
                // Si el primer número es igual, comparar por el segundo (si existe)
                if (numerosA.length > 1 && numerosB.length > 1) {
                    return numerosA[1] - numerosB[1];
                }
                // Si solo uno tiene segundo número, el que tiene más números va después
                return numerosA.length - numerosB.length;
            }
            
            // Si solo uno tiene números, el que tiene números va primero
            if (numerosA.length > 0 && numerosB.length === 0) {
                return -1;
            }
            if (numerosA.length === 0 && numerosB.length > 0) {
            ...
            // Si ninguno tiene números, mantener orden original (retornar 0)
            return 0;
        });
    }
    ...
    function generarTablaPorTipoTrampa(semana, fecha, tipoTrampa, nombreTipoTrampa) {
        ...
        const palabrasClave = {
            ...
            'luz_uv': ['luz', 'uv'],
            'feromona_gorgojo': ['feromona', 'gorgojo'],
            'feromona_palomilla': ['feromona', 'palomilla'],
            'equipo_sonico': ['sonico', 'equipo_sonico'],
            'globo_terror': ['globo', 'terror'],
            'atrayente_pulgas': ['atrayente', 'pulga'],
            'atrayente_chinches': ['atrayente', 'chinche']
        };
        
        // Filtrar trampas por tipo usando palabras clave
        let trampasFiltradas = trampasData.filter(t => {
        ...
        // Ordenar trampas por números en el nombre (si contiene números)
        trampasFiltradas = ordenarTrampasPorNumero(trampasFiltradas);
    ...
    // Función para generar tabla de Equipo Sónico (columnas especiales: Funcionamiento, Condición de Equipo)
    function generarTablaEquipoSonico(semana, fecha) {
        const palabrasClave = ['sonico'];
        
        let trampasFiltradas = trampasData.filter(t => {
            const tipoTrampaNormalizado = normalizarTipoTrampa(t.tipo || '');
            return palabrasClave.every(clave => tipoTrampaNormalizado.includes(clave));
        });
        ...
        let html = `
            <div class="table-header" style="margin-top: 30px;">
                <h2 style="font-size: 16px; font-weight: bold; margin-bottom: 8px;">EQUIPO SÓNICO</h2>
            </div>
        ...
        trampasFiltradas.forEach((trampa, index) => {
            const nombreTrampa = trampa.nombre || trampa.id_trampa || 'T' + trampa.id;
            const ubicacion = trampa.ubicacion || 'Sin ubicación';
            const rowId = `semana-${semana}-trampa-${trampa.id}-equipo_sonico`;
            ...
            html += `
                        <tr>
                            ...
                            <td>
                                <select 
                                    id="${rowId}-funcionamiento"
                                    class="estado-input"
                                    data-semana="${semana}"
                                    data-trampa-id="${trampa.id}"
                                    data-tipo-trampa="equipo_sonico"
                                    data-campo="funcionamiento"
                                    data-row-index="${index}"
    ...
    // Función para generar tabla de Globo de Terror (solo condición de equipo, sin captura de organismos)
    function generarTablaGloboTerror(semana, fecha) {
        const palabrasClave = ['globo', 'terror'];
        ...
        let html = `
            <div class="table-header" style="margin-top: 30px;">
                <h2 style="font-size: 16px; font-weight: bold; margin-bottom: 8px;">GLOBO DE TERROR</h2>
            </div>
        ...
        trampasFiltradas.forEach((trampa, index) => {
            const nombreTrampa = trampa.nombre || trampa.id_trampa || 'T' + trampa.id;
            const ubicacion = trampa.ubicacion || 'Sin ubicación';
            const rowId = `semana-${semana}-trampa-${trampa.id}-globo_terror`;
    ...
    // Función para generar tabla de Trampa Atrayente Pulgas (solo cantidad de organismos)
    function generarTablaAtrayentePulgas(semana, fecha) {
        const palabrasClave = ['atrayente', 'pulga'];
        ...
        let html = `
            <div class="table-header" style="margin-top: 30px;">
                <h2 style="font-size: 16px; font-weight: bold; margin-bottom: 8px;">TRAMPA ATRAYENTE PULGAS</h2>
            </div>
        ...
        trampasFiltradas.forEach((trampa, index) => {
            const nombreTrampa = trampa.nombre || trampa.id_trampa || 'T' + trampa.id;
            const ubicacion = trampa.ubicacion || 'Sin ubicación';
            const rowId = `semana-${semana}-trampa-${trampa.id}-atrayente_pulgas`;
                        ...
                        <tr>
                            ...
                            <td>
                                <input 
                                    ...
                                    data-semana="${semana}"
                                    data-trampa-id="${trampa.id}"
                                    data-tipo-trampa="atrayente_pulgas"
                                    data-campo="cantidad"
                                    data-row-index="${index}"
                                    value="0" 
                                    min="0"
    ...
    // Función para generar tabla de Trampa Atrayente Chinches (solo cantidad de organismos)
    function generarTablaAtrayenteChinches(semana, fecha) {
        const palabrasClave = ['atrayente', 'chinche'];
        ...
        let html = `
            <div class="table-header" style="margin-top: 30px;">
                <h2 style="font-size: 16px; font-weight: bold; margin-bottom: 8px;">TRAMPA ATRAYENTE CHINCHES</h2>
            </div>
        ...
        trampasFiltradas.forEach((trampa, index) => {
            const nombreTrampa = trampa.nombre || trampa.id_trampa || 'T' + trampa.id;
            const ubicacion = trampa.ubicacion || 'Sin ubicación';
            const rowId = `semana-${semana}-trampa-${trampa.id}-atrayente_chinches`;
                        ...
                        <tr>
                            ...
                            <td>
                                <input 
                                    ...
                                    data-semana="${semana}"
                                    data-trampa-id="${trampa.id}"
                                    data-tipo-trampa="atrayente_chinches"
                                    data-campo="cantidad"
                                    data-row-index="${index}"
                                    value="0" 
                                    min="0"
    ...
    // Función para generar tabla de otras trampas (que no tienen tabla específica)
    function generarTablaOtrasTrampas(semana, fecha) {
        // Tipos de trampa que ya tienen su propia tabla o no deben aparecer aquí
        const tiposConTablaPropia = [
            'edc_quimicas', 'edc_adhesivas', 'luz_uv', 
            'feromona_gorgojo', 'feromona_palomilla',
            // 'equipo_sonico' y 'globo_terror' NO aparecen aquí (tienen vista de Estado de Trampas)
            'atrayente_pulgas', 'atrayente_chinches'
        ];
        
        // Tipos que no deben aparecer en las tablas de incidencias (tienen su propia vista)
        const tiposExcluidos = ['equipo_sonico', 'globo_terror'];
        
        // Palabras clave para identificar cada tipo de trampa
        const palabrasClaveTodos = {
            'edc_quimicas': ['edc', 'quimica'],
            'edc_adhesivas': ['edc', 'adhesiva'],
            'luz_uv': ['luz', 'uv'],
            'feromona_gorgojo': ['feromona', 'gorgojo'],
            'feromona_palomilla': ['feromona', 'palomilla'],
            'equipo_sonico': ['sonico'],
            'globo_terror': ['globo', 'terror'],
            'atrayente_pulgas': ['atrayente', 'pulga'],
            'atrayente_chinches': ['atrayente', 'chinche']
        };
        ...
        let trampasFiltradas = trampasData.filter(t => {
            ...
            // Verificar si NO es de ninguno de los tipos con tabla propia
            const tieneTablaPropia = tiposConTablaPropia.some(tipoPrincipal => {
                const claves = palabrasClaveTodos[tipoPrincipal] || [];
                const todasLasClavesPresentes = claves.every(clave => 
                    tipoTrampaNormalizado.includes(clave)
                );
            ...
            // Verificar si está excluido (tiene su propia vista)
            const estaExcluido = tiposExcluidos.some(tipoExcluido => {
                const claves = palabrasClaveTodos[tipoExcluido] || [];
                return claves.every(clave => tipoTrampaNormalizado.includes(clave));
            });
            
            return !tieneTablaPropia && !estaExcluido;
        });
    ...
    function generarTablaHTML(semana, fecha) {
        ...
        // Generar tabla para EDC Adhesivas (EDS Físicas en la imagen)
        ...
        // Generar tabla para Trampa de Feromona Gorgojo
        html += generarTablaPorTipoTrampa(semana, fecha, 'feromona_gorgojo', 'TRAMPA DE FEROMONA DE GORGOJO');
        console.log('Tabla Trampa de Feromona Gorgojo generada');
        
        // Generar tabla para Trampa de Feromona Palomilla
        html += generarTablaPorTipoTrampa(semana, fecha, 'feromona_palomilla', 'TRAMPA DE FEROMONA DE PALOMILLA');
        console.log('Tabla Trampa de Feromona Palomilla generada');
        
        // NOTA: Equipo Sónico y Globo de Terror NO aparecen aquí porque tienen su propia vista de Estado de Trampas
        
        // Generar tabla para Trampa Atrayente Pulgas (tabla especial)
        html += generarTablaAtrayentePulgas(semana, fecha);
        console.log('Tabla Trampa Atrayente Pulgas generada');
        
        // Generar tabla para Trampa Atrayente Chinches (tabla especial)
        html += generarTablaAtrayenteChinches(semana, fecha);
        console.log('Tabla Trampa Atrayente Chinches generada');
        
        // Generar tabla para otras trampas (que no sean de los tipos principales)
    ...
    function actualizarIncidencias(input) {
        ...
        // Verificar si es un input especial de Atrayente Chinches o Pulgas (tiene data-campo="cantidad")
        const campoEspecial = input.dataset.campo;
        const tipoTrampaEspecial = input.dataset.tipoTrampa;
        
        let tipoIndex;
        let tipoInsecto;
        let mapeo;
        
        if (campoEspecial === 'cantidad' && tipoTrampaEspecial) {
            // Es un input especial de Atrayente Chinches o Pulgas
            if (tipoTrampaEspecial === 'atrayente_chinches') {
                tipoInsecto = 'Chinche';
            } else if (tipoTrampaEspecial === 'atrayente_pulgas') {
                tipoInsecto = 'Pulga';
            } else {
                return; // Tipo de trampa especial no reconocido
            }
            
            mapeo = mapeoTiposInsectos[tipoInsecto];
            if (!mapeo) return;
            
            // Obtener el índice del tipo de insecto
            tipoIndex = tiposInsectos.indexOf(tipoInsecto);
            if (tipoIndex < 0) return;
        } else {
            // Input normal con data-tipo-index
            tipoIndex = parseInt(input.dataset.tipoIndex);
            if (isNaN(tipoIndex)) return;
            
            tipoInsecto = tiposInsectos[tipoIndex];
            mapeo = mapeoTiposInsectos[tipoInsecto];
            if (!mapeo) return;
        }
                    ...
                    <tr data-incidencia-id="${incidencia.id}">
                        ...
                        <td>
                            <select class="editable-tipo-plaga" data-incidencia-id="${incidencia.id}" data-semana="${semana}">
                                ...
                                <!-- Organismos especiales para trampas específicas -->
                                <option value="pulga" ${incidencia.tipo_plaga === 'pulga' ? 'selected' : ''}>Pulga</option>
                                <option value="chinche" ${incidencia.tipo_plaga === 'chinche' ? 'selected' : ''}>Chinche</option>
                                <!-- Organismos para Trampa de Feromona Gorgojo -->
                                <option value="sithopilus" ${incidencia.tipo_plaga === 'sithopilus' ? 'selected' : ''}>Sithopilus sp.</option>
                                <option value="lasioderma" ${incidencia.tipo_plaga === 'lasioderma' ? 'selected' : ''}>Lasioderma sp.</option>
                                <option value="tribolium" ${incidencia.tipo_plaga === 'tribolium' ? 'selected' : ''}>Tribolium sp.</option>
                                <option value="stegobium" ${incidencia.tipo_plaga === 'stegobium' ? 'selected' : ''}>Stegobium sp.</option>
                                <option value="rizopertha" ${incidencia.tipo_plaga === 'rizopertha' ? 'selected' : ''}>Rizopertha sp.</option>
                                <option value="orizaephilus" ${incidencia.tipo_plaga === 'orizaephilus' ? 'selected' : ''}>Orizaephilus sp.</option>
                                <option value="trogoderma" ${incidencia.tipo_plaga === 'trogoderma' ? 'selected' : ''}>Trogoderma sp.</option>
                                <option value="dermestes" ${incidencia.tipo_plaga === 'dermestes' ? 'selected' : ''}>Dermestes sp.</option>
                                <option value="tenebroides" ${incidencia.tipo_plaga === 'tenebroides' ? 'selected' : ''}>Tenebroides sp.</option>
                                <option value="otro_gorgojo" ${incidencia.tipo_plaga === 'otro_gorgojo' ? 'selected' : ''}>Otro (gorgojo)</option>
                                <!-- Organismos para Trampa de Feromona Palomilla -->
                                <option value="plodia" ${incidencia.tipo_plaga === 'plodia' ? 'selected' : ''}>Plodia</option>
                                <option value="ephestia" ${incidencia.tipo_plaga === 'ephestia' ? 'selected' : ''}>Ephestia</option>
                                <option value="sitotroga" ${incidencia.tipo_plaga === 'sitotroga' ? 'selected' : ''}>Sitotroga</option>
                                <option value="otra_palomilla" ${incidencia.tipo_plaga === 'otra_palomilla' ? 'selected' : ''}>Otra (palomilla)</option>
                            </select>
                        </td>
                        <td>
    ...
    function guardarTodasIncidencias(semana) {
        ...
        const payload = {
            incidencias: incidencias.map(inc => ({
                trampa_id: inc.tipo_incidencia === 'Hallazgo' ? '' : inc.trampa_id,
                zona: inc.zona || '',
                plano_id: planoData.id,
                sede_id: planoData.sede_id,
                tipo_plaga: inc.tipo_plaga,
                tipo_incidencia: inc.tipo_incidencia,
                tipo_insecto: inc.tipo_insecto,
                cantidad_organismos: inc.cantidad_organismos,
                notas: inc.notas || '',
                inspector: inc.inspector || inspectorPorDefecto,
                fecha_incidencia: inc.fecha_incidencia + ' 00:00:00'
            }))
        };

        fetch('<?= base_url('blueprints/guardar_incidencias_batch') ?>', {
            method: 'POST',
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
        ...
        .then(response => response.json())
        .then(data => {
            let mensaje = `${data.guardadas} incidencias guardadas correctamente`;
            if (data.errores > 0) {
                mensaje += `, ${data.errores} errores`;
                if (data.detalle_errores && data.detalle_errores.length > 0) {
                    const detalles = data.detalle_errores.slice(0, 3).map(e => `Incidencia ${e.index + 1}: ${e.message}`);
                    mensaje += '\n' + detalles.join('\n');
                    if (data.detalle_errores.length > 3) {
                        mensaje += `\n... y ${data.detalle_errores.length - 3} más`;
                    }
                }
                alert(mensaje);
            } else {
