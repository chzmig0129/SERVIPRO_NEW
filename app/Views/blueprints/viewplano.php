...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex gap-3">
            ...
            <a href="<?= base_url('blueprints/estadoTrampas/' . $plano['id']) ?>" class="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700">
                ...
                <div class="space-y-2">
                    ...
                    <div class="dropdown relative">
                        ...
                        <div class="trap-menu hidden absolute w-full bg-white border rounded-lg shadow-lg" style="z-index: 1000;">
                            ...
                            <a href="#" class="dropdown-item block px-4 py-2 hover:bg-gray-100" data-trap-type="feromona_palomilla">
                                <i class="fas fa-feather mr-2"></i> Trampa de Feromona Palomilla
                            </a>
                ...
                <table class="tabla-incidencias w-full text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200" id="tbodyIncidencias">
                        <?php foreach ($incidencias as $incidencia): ?>
                            ...
                            // Para hallazgos, mostrar la zona extraída de las notas
                            if (!empty($incidencia['trampa_nombre'])) {
                                $nombreTrampa = $incidencia['trampa_nombre'];
                            } elseif (!empty($incidencia['id_trampa'])) {
                                $nombreTrampa = $incidencia['id_trampa'];
                            } elseif (!empty($incidencia['trampa_ubicacion'])) {
                                // Para hallazgos, mostrar la zona
                                $nombreTrampa = $incidencia['trampa_ubicacion'];
                            } else {
                                $nombreTrampa = 'N/A';
                            }
                            ?>
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col overflow-hidden" style="width: 90%; max-width: 1152px;">
            <div class="overflow-y-auto overflow-x-hidden" style="max-height: calc(90vh - 0px);">
                ...
                <form id="formIncidencia">
                    <!-- Contexto del plano (necesario para guardar plano_id en incidencias, especialmente si la trampa no lo trae por datos viejos) -->
                    <input type="hidden" name="plano_id" id="plano_id" value="<?= esc($plano['id'] ?? '') ?>">
                    <input type="hidden" name="sede_id" id="sede_id" value="<?= esc($sede['id'] ?? '') ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (estadoGuardado && estadoGuardado.imagen) {
                ...
                planoImage.onload = function() {
                    ...
                    const trampasParaCargar = trampasDesdeDB.length > 0 ? trampasDesdeDB.map(t => ({
                        db_id: t.id, // ID único de la BD (PRIMARY KEY) - NUNCA se repite
                        id: t.id_trampa || t.id,
        ...
        function limpiarEstadoCompleto() {
            ...
            // Eliminar trampas de la BD para este plano específico
            fetch('<?= base_url("blueprints/eliminar_trampas_plano") ?>', {
                method: 'POST',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    console.log('Trampas eliminadas de la BD:', data.message);
                } else {
                    console.error('Error al eliminar trampas de la BD:', data.message);
                }
            })
            .catch(error => {
        ...
        // Función auxiliar para normalizar el tipo de trampa al código interno
        // Esto permite que el switch funcione tanto con códigos internos como con nombres legibles
        function normalizarTipoTrampa(tipo) {
            if (!tipo) return 'default';
            
            // Mapeo de nombres legibles a códigos internos
            const mapeo = {
                'EDC Químicas': 'edc_quimicas',
                'EDC Adhesivas': 'edc_adhesivas',
                'Equipo de Luz UV': 'luz_uv',
                'Equipo Sónico': 'equipo_sonico',
                'Globo terror': 'globo_terror',
                'Trampa de Feromona Gorgojo': 'feromona_gorgojo',
                'Trampa de Feromona Palomilla': 'feromona_palomilla',
                'Trampa atrayente chinches': 'atrayente_chinches',
                'Trampa atrayente pulgas': 'atrayente_pulgas',
                'Trampa feromonas picudo rojo': 'feromona_picudo'
            };
            
            // Si ya es un código interno, devolverlo tal cual
            if (mapeo[tipo]) {
                return mapeo[tipo];
            }
            
            // Si no está en el mapeo, devolver el tipo original (puede ser un código interno)
            return tipo;
        }
        ...
        function marcarTrampa(punto) {
            ...
            // Normalizar el tipo primero para que funcione tanto con códigos internos como con nombres legibles
            const tipoNormalizado = normalizarTipoTrampa(punto.tipo);
            ...
            marcador.addEventListener('contextmenu', function(e) {
                e.preventDefault();
                if (confirm('Esta accion eliminara la trampa de forma PERMANENTE, incluyendo todas sus incidencias y registros asociados. ¿Esta seguro de que desea continuar?')) {
                    const index = parseInt(this.dataset.index);
                    const marker = this;
                    const punto = window.puntos[index];
                    const dbId = punto ? punto.db_id : null;
                    if (dbId && !String(dbId).startsWith('TEMP-')) {
                        fetch('<?= base_url('blueprints/eliminar_trampa') ?>', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded',
                                'X-Requested-With': 'XMLHttpRequest'
                            },
                            body: 'db_id=' + encodeURIComponent(dbId)
                        })
                        .then(response => response.json())
                        .then(data => {
                            if (data.success) {
                        ...
                        .catch(error => {
                            ...
                            alert('Error de conexion al eliminar la trampa');
                        });
                    } else {
        ...
        window.cerrarModalEditarId = function() {
            ...
            formData.append('db_id', window.trampaSeleccionadaParaEdicion.db_id);
        ...
        function getTipoTrampa(tipo) {
            const tipos = {
                ...
                'Trampa de Feromona Palomilla': 'Trampa de Feromona Palomilla',
                ...
                'feromona_palomilla': 'Trampa de Feromona Palomilla',
        ...
        function guardarTrampaEnBD(trampa) {
            ...
            if (trampa.id && !trampa.id.startsWith('TEMP-')) {
                formData.append('id_trampa', trampa.id_trampa || trampa.id);
            }
            
            // Si la trampa tiene un db_id (ID único de la BD), enviarlo para identificar movimientos
            if (trampa.db_id) {
                formData.append('db_id', trampa.db_id);
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (data.trampa && data.trampa.id_trampa) {
                        ...
                        trampa.db_id = data.trampa.db_id; // ID único de la BD (PRIMARY KEY)
                        ...
                        // Normalizar el tipo para mantener consistencia en memoria
                        trampa.tipo = getTipoTrampa(trampa.tipo);
        ...
        document.addEventListener('mouseup', function() {
            ...
            if (nuevaZona) {
                ...
                formData.append('sede_id', <?= $sede['id'] ?>);
                formData.append('plano_id', <?= $plano['id'] ?>);
                // Normalizar el tipo antes de enviarlo para mantener consistencia en la BD
                formData.append('tipo', getTipoTrampa(trampaOriginal.tipo));
                formData.append('ubicacion', nuevaZona);
                formData.append('coordenada_x', newX);
                formData.append('coordenada_y', newY);
                formData.append('id_trampa', trampaOriginal.id_trampa || trampaOriginal.id);
                
                // Enviar db_id para identificar la trampa específica a mover
                if (trampaOriginal.db_id) {
                    formData.append('db_id', trampaOriginal.db_id);
                }
                ...
                .then(data => {
                    if (data.success) {
                        // Sincronizar datos del servidor al objeto en memoria
                        if (data.trampa_id) {
                            trampaOriginal.db_id = data.trampa_id;
                        }
                        if (data.id_trampa) {
                            trampaOriginal.id_trampa = data.id_trampa;
                        }
                        if (data.nombre) {
                            trampaOriginal.nombre = data.nombre;
                        }
