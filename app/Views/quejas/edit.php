...
<div class="bg-gradient-to-r from-blue-500 to-blue-700 rounded-xl p-6 text-white shadow-lg text-center mb-6">
    <div class="flex flex-col items-center justify-center">
        ...
        <p class="text-blue-100">Modifica los datos de la queja</p>
    </div>
</div>

<div class="bg-white rounded-lg shadow-sm p-6">
    <form action="<?= site_url('quejas/update/' . $queja['id']) ?>" method="post" enctype="multipart/form-data" class="space-y-6">
        <?= csrf_field() ?>
        ...
        <div>
            ...
            <input type="date" 
                   name="fecha" 
                   id="fecha" 
                   value="<?= old('fecha', date('Y-m-d', strtotime($queja['fecha']))) ?>"
        ...
        <div>
            ...
            <select name="sede_id" 
                ...
                <?php foreach ($sedes as $sede): ?>
                    <option value="<?= $sede['id'] ?>" <?= (old('sede_id', $queja['sede_id']) == $sede['id']) ? 'selected' : '' ?>>
                        <?= esc($sede['nombre']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <div>
            ...
            <input type="text" 
                   name="insecto" 
                   id="insecto" 
                   value="<?= old('insecto', $queja['insecto']) ?>"
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                   required>
        </div>
        ...
        <div>
            ...
            <input type="text" 
                   name="ubicacion" 
                   id="ubicacion" 
                   value="<?= old('ubicacion', $queja['ubicacion']) ?>"
                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                   required>
        </div>
            ...
            <?php if (!empty($queja['archivo'])): ?>
                <div class="mb-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div class="flex items-center">
                        ...
                        <span class="text-sm text-green-700">Archivo actual: <?= esc($queja['archivo']) ?></span>
                        <a href="<?= base_url('public/uploads/quejas/' . $queja['archivo']) ?>" target="_blank" class="ml-auto text-green-600 hover:text-green-800">
            ...
            <div class="flex items-center justify-center w-full">
                <label for="archivo" class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                        ...
                        <p class="mb-2 text-sm text-gray-500"><span class="font-semibold">Haga clic para cargar</span> un nuevo archivo</p>
        ...
        <div>
            ...
            <select name="clasificacion" 
                ...
                <option value="Crítico" <?= old('clasificacion', $queja['clasificacion']) == 'Crítico' ? 'selected' : '' ?>>Crítico</option>
                <option value="Alto" <?= old('clasificacion', $queja['clasificacion']) == 'Alto' ? 'selected' : '' ?>>Alto</option>
                <option value="Medio" <?= old('clasificacion', $queja['clasificacion']) == 'Medio' ? 'selected' : '' ?>>Medio</option>
                <option value="Bajo" <?= old('clasificacion', $queja['clasificacion']) == 'Bajo' ? 'selected' : '' ?>>Bajo</option>
            </select>
        </div>
            ...
            <div class="flex gap-4">
                <label class="inline-flex items-center">
                    <input type="radio" 
                           name="estado" 
                           value="Vivo" 
                           <?= old('estado', $queja['estado'] ?? 'Vivo') == 'Vivo' ? 'checked' : '' ?> 
                ...
                <label class="inline-flex items-center">
                    <input type="radio" 
                           name="estado" 
                           value="Muerto" 
                           <?= old('estado', $queja['estado'] ?? '') == 'Muerto' ? 'checked' : '' ?> 
                           class="form-radio h-4 w-4 text-blue-600" 
                           required>
        ...
        <div>
            ...
            <select name="estado_queja" 
                ...
                <option value="Pendiente" <?= old('estado_queja', $queja['estado_queja'] ?? '') == 'Pendiente' ? 'selected' : '' ?>>Pendiente</option>
                <option value="En Proceso" <?= old('estado_queja', $queja['estado_queja'] ?? '') == 'En Proceso' ? 'selected' : '' ?>>En Proceso</option>
                <option value="Resuelta" <?= old('estado_queja', $queja['estado_queja'] ?? '') == 'Resuelta' ? 'selected' : '' ?>>Resuelta</option>
                <option value="Cerrada" <?= old('estado_queja', $queja['estado_queja'] ?? '') == 'Cerrada' ? 'selected' : '' ?>>Cerrada</option>
            </select>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Si hay un valor guardado (valor actual de la queja), seleccionar la opción correspondiente
    ...
    archivoInput.addEventListener('change', function(e) {
        ...
        if (file) {
            ...
            // Mostrar preview
            archivoName.textContent = file.name + ' (reemplazará el archivo actual)';
