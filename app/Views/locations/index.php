...
<div class="space-y-6 max-w-7xl mx-auto px-4">
    ...
    <!-- SECCION VOLADORES -->
    <!-- ============================================ -->
    <div class='bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-4 mt-8 mb-4'>
        <div class='flex items-center gap-3'>
            <span class='px-3 py-1 bg-blue-500 text-white rounded-full text-sm font-bold'>VOLADORES</span>
            <h3 class='text-xl font-bold text-blue-900'>Organismos Voladores</h3>
        </div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Distribución de Plagas - Voladores</h4>
        <div style='height:400px;'><canvas id='pieVoladores'></canvas></div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 mt-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Trampas con Mayor Captura - Voladores</h4>
        <div style='height:400px;'><canvas id='trampasCapturaVoladores'></canvas></div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 mt-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Áreas que Presentaron Capturas - Voladores</h4>
        <div style='height:500px;'><canvas id='areasCapturaVoladores'></canvas></div>
    </div>
    <!-- ============================================ -->
    <!-- SECCION RASTREROS -->
    <!-- ============================================ -->
    <div class='bg-gradient-to-r from-green-50 to-green-100 rounded-lg p-4 mt-8 mb-4'>
        <div class='flex items-center gap-3'>
            <span class='px-3 py-1 bg-green-500 text-white rounded-full text-sm font-bold'>RASTREROS</span>
            <h3 class='text-xl font-bold text-green-900'>Organismos Rastreros</h3>
        </div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Distribución de Plagas - Rastreros</h4>
        <div style='height:400px;'><canvas id='pieRastreros'></canvas></div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 mt-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Trampas con Mayor Captura - Rastreros</h4>
        <div style='height:400px;'><canvas id='trampasCapturaRastreros'></canvas></div>
    </div>
    <div class='bg-white rounded-lg shadow-sm p-6 mt-6 relative'>
        <h4 class='text-lg font-semibold text-gray-800 mb-4'>Áreas que Presentaron Capturas - Rastreros</h4>
        <div style='height:500px;'><canvas id='areasCapturaRastreros'></canvas></div>
    </div>
...
function generarPDFConDOM2PDF() {
    try {
        ...
        const chartsConfig = {
            ...
            'pieVoladores': { id: 'pieVoladores', title: 'Distribucion de Plagas - Voladores' },
            'pieRastreros': { id: 'pieRastreros', title: 'Distribucion de Plagas - Rastreros' },
            'trampasCapturaVoladores': { id: 'trampasCapturaVoladores', title: 'Trampas con Mayor Captura - Voladores' },
            'trampasCapturaRastreros': { id: 'trampasCapturaRastreros', title: 'Trampas con Mayor Captura - Rastreros' },
            'areasCapturaVoladores': { id: 'areasCapturaVoladores', title: 'Areas que Presentaron Capturas - Voladores' },
            'areasCapturaRastreros': { id: 'areasCapturaRastreros', title: 'Areas que Presentaron Capturas - Rastreros' }
        };
            ...
            options: {
                ...
                scales: {
                    ...
                    y: { 
                        ...
                        max: Math.max(50, Math.ceil((maxTotal * 1.2) / 50) * 50), // al menos 50 o redondeado al múltiplo de 50 más cercano
                        title: { 
                            display: true, 
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const colores = generateDistinctColorPalette(etiquetas.length);
...
<script>
    ...
    function inicializarGraficaTipoIncidencia() {
        ...
        const totalConsumo = <?= $totalConsumo ?? 0; ?>;
        ...
        try {
            new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: ['Hallazgo', 'Captura', 'Consumo'],
                    datasets: [{
                        label: 'Cantidad de Incidencias',
                        data: [totalHallazgos, totalCapturas, totalConsumo],
                        backgroundColor: [
                            'rgba(40, 167, 69, 0.8)',  // Verde para Hallazgo
                            'rgba(54, 162, 235, 0.8)',  // Azul para Captura
                            'rgba(255, 206, 86, 0.8)'   // Amarillo para Consumo
                        ],
                        borderColor: [
                            'rgba(40, 167, 69, 1)',
                            'rgba(54, 162, 235, 1)',
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function actualizarGraficaIncidencia(plagaSeleccionada) {
            ...
            const colors = generateDistinctColorPalette(labels.length);
    ...
    // Función helper para convertir HSL a RGB
    ...
    // Función para generar paleta de colores multicolor para gráficos de pastel
    function generateDistinctColorPalette(count) {
        const distinctColors = [
            'rgba(220, 50, 50, 0.8)',    // Rojo
            'rgba(54, 130, 220, 0.8)',   // Azul
            'rgba(50, 180, 80, 0.8)',    // Verde
            'rgba(255, 150, 30, 0.8)',   // Naranja
            'rgba(150, 60, 200, 0.8)',   // Morado
            'rgba(230, 210, 30, 0.8)',   // Amarillo
            'rgba(30, 190, 190, 0.8)',   // Cian
            'rgba(230, 80, 160, 0.8)',   // Rosa
            'rgba(100, 60, 30, 0.8)',    // Café
            'rgba(80, 200, 140, 0.8)',   // Verde menta
            'rgba(255, 100, 60, 0.8)',   // Naranja rojizo
            'rgba(60, 80, 200, 0.8)',    // Azul índigo
            'rgba(180, 220, 50, 0.8)',   // Lima
            'rgba(200, 50, 120, 0.8)',   // Magenta
            'rgba(30, 140, 100, 0.8)',   // Verde esmeralda
            'rgba(120, 120, 30, 0.8)'    // Oliva
        ];
        ...
        for (let i = 0; i < count; i++) {
            if (i < distinctColors.length) {
                ...
                // Generar colores adicionales distribuidos por el espectro HSL
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: { 
                            beginAtZero: true,
                            min: 0,
                            max: Math.max(50, Math.ceil((Math.max(...data) * 1.3) / 50) * 50), // Al menos 50 o redondeado al múltiplo de 50 más cercano
                            title: { 
                                display: true,
                    ...
                    tooltip: {
                        callbacks: {
                            ...
                            afterBody: function(tooltipItems) {
                                ...
                                return ['Total en esta ubicacion: ' + totalReal + ' organismos'];
                            }
                        },
                        backgroundColor: 'rgba(30, 41, 59, 0.9)',
                        titleFont: {
                ...
                scales: {
                    ...
                    y: {
                        ...
                        max: function(context) {
                            ...
                            return Math.ceil((maxStackedValue * 1.15) / 50) * 50;
                        },
                        title: {
                            display: true,
                            text: 'Cantidad de Organismos',
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function actualizarGraficaTipoIncidencia() {
            ...
            // Contar hallazgos, capturas y consumo de las filas visibles
                ...
                filasIncidencias.forEach(fila => {
                    ...
                    if (fila.style.display !== 'none') {
                        ...
                        if (tipoIncidencia.toLowerCase() === 'hallazgo') {
                            ...
                            totalCapturas++;
                        } else if (tipoIncidencia.toLowerCase() === 'consumo') {
                            totalConsumo++;
                        }
                    }
                });
            ...
            if (chartInstance) {
                ...
                chartInstance.data.labels = ['Hallazgo', 'Captura', 'Consumo'];
                chartInstance.data.datasets[0].data = [totalHallazgos, totalCapturas, totalConsumo];
                // Actualizar colores si es necesario
                if (chartInstance.data.datasets[0].backgroundColor.length < 3) {
...
<!-- Script para gráficas de Voladores y Rastreros -->
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // =============================================
        (function() {
            const incidenciasPorTipoPlagaVoladores = <?= json_encode($incidenciasPorTipoPlagaVoladores ?? []) ?>;

            let incidenciasMap = {};
            incidenciasPorTipoPlagaVoladores.forEach(item => {
                let plaga = item.tipo_plaga || "Desconocida";
                let total = parseInt(item.total, 10) || 0;
                if (!incidenciasMap[plaga]) incidenciasMap[plaga] = 0;
                incidenciasMap[plaga] += total;
            });
            ...
            const canvas = document.getElementById('pieVoladores');
            if (!canvas) return;
            const ctx = canvas.getContext('2d');
            new Chart(ctx, {
                type: 'pie',
                    ...
                    plugins: {
                        legend: {
                            ...
                            labels: {
                                ...
                                usePointStyle: true,
                                pointStyle: 'circle',
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 11, weight: 'bold' }
                            }
                        },
                        title: {
                            display: true,
                            text: 'DISTRIBUCIÓN DE PLAGAS - VOLADORES',
                        ...
                        datalabels: {
                            ...
                            font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", weight: 'bold', size: 12 },
                            textStrokeColor: 'rgba(0,0,0,0.5)',
                            textStrokeWidth: 2,
                            textShadowBlur: 3,
                            textShadowColor: 'rgba(0,0,0,0.5)'
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    const label = context.label || '';
                                    const value = context.raw;
                                    const total = context.chart.data.datasets[0].data.reduce((acc, d) => acc + d, 0);
                                    const percentage = total > 0 ? (value / total) * 100 : 0;
                                    const percentageText = percentage < 1 && percentage > 0 ? '>1%' : Math.round(percentage) + '%';
                                    return `${label}: ${value} (${percentageText})`;
                                }
                            },
                            backgroundColor: 'rgba(30,41,59,0.8)',
        ...
        // =============================================
        (function() {
            const incidenciasPorTipoPlagaRastreros = <?= json_encode($incidenciasPorTipoPlagaRastreros ?? []) ?>;

            let incidenciasMap = {};
            incidenciasPorTipoPlagaRastreros.forEach(item => {
                let plaga = item.tipo_plaga || "Desconocida";
                let total = parseInt(item.total, 10) || 0;
                if (!incidenciasMap[plaga]) incidenciasMap[plaga] = 0;
                incidenciasMap[plaga] += total;
            ...
            const canvas = document.getElementById('pieRastreros');
            if (!canvas) return;
            const ctx = canvas.getContext('2d');
            new Chart(ctx, {
                type: 'pie',
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'DISTRIBUCIÓN DE PLAGAS - RASTREROS',
                            font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 16, weight: 'bold' },
                            padding: { top: 10, bottom: 20 }
                        },
                        datalabels: {
        ...
        // =============================================
        // 3. TRAMPAS CAPTURA VOLADORES
        // =============================================
        (function() {
            const todasTrampasCapturaVoladores = <?= json_encode($todasTrampasCapturaVoladores ?? []) ?>;

            if (!todasTrampasCapturaVoladores || todasTrampasCapturaVoladores.length === 0) {
                const canvas = document.getElementById('trampasCapturaVoladores');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    ctx.save();
                    ctx.textAlign = 'center';
            ...
            // Agregar cantidades por trampa
            const trampasSumadas = {};
            todasTrampasCapturaVoladores.forEach(item => {
                const key = `${item.id_trampa} (${item.ubicacion})`;
                if (!trampasSumadas[key]) {
                    trampasSumadas[key] = { id_trampa: item.id_trampa, trampa_nombre: item.trampa_nombre, ubicacion: item.ubicacion, cantidad_total: 0, total_capturas: 0 };
                }
                trampasSumadas[key].cantidad_total += parseInt(item.cantidad_total) || 0;
                trampasSumadas[key].total_capturas += parseInt(item.total_capturas) || 0;
            });

            let datosFiltrados = Object.values(trampasSumadas);
            datosFiltrados.sort((a, b) => parseInt(b.cantidad_total) - parseInt(a.cantidad_total));
            datosFiltrados = datosFiltrados.slice(0, 10);
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        label: 'Cantidad de Organismos',
                        data: data.map(value => value > 0 ? Math.max(value, 0.5) : 0),
                        backgroundColor: generateDistinctColorPalette(data.length),
                        borderColor: generateDistinctColorPalette(data.length).map(c => c.replace('0.8', '1')),
                        borderWidth: 1,
                        barPercentage: 0.8,
                        categoryPercentage: 0.9,
                        minBarLength: 5
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'TRAMPAS CON MAYOR CAPTURA - VOLADORES',
                        ...
                        tooltip: {
                            callbacks: {
                                ...
                                label: function(context) {
                                    return 'Organismos: ' + (context.parsed.y !== null ? context.parsed.y : '');
                                },
                                afterLabel: function(context) {
                                    const index = context.dataIndex;
                                    const item = datosFiltrados[index];
                                    return [`Ubicación: ${item.ubicacion}`, `Frecuencia: ${item.total_capturas} capturas`];
                                }
                            }
                        }
                    },
                    scales: {
                        x: {
                            title: { display: false },
                            grid: { display: false },
                            ticks: { autoSkip: true, maxTicksLimit: 10, maxRotation: 45, minRotation: 45, font: { size: 10 } }
                        },
                        y: {
                            beginAtZero: true,
                            min: 0,
                            max: Math.max(50, Math.ceil((maxVal * 1.3) / 50) * 50),
                            title: { display: true, text: 'Cantidad de organismos capturados' },
                            grid: { borderDash: [2, 2] },
                            ticks: { precision: 0, stepSize: 50 }
                        }
                    }
                }
            });
        ...
        // =============================================
        // 4. TRAMPAS CAPTURA RASTREROS
        // =============================================
        (function() {
            const todasTrampasCapturaRastreros = <?= json_encode($todasTrampasCapturaRastreros ?? []) ?>;

            if (!todasTrampasCapturaRastreros || todasTrampasCapturaRastreros.length === 0) {
                const canvas = document.getElementById('trampasCapturaRastreros');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    ctx.save();
                    ctx.textAlign = 'center';
            ...
            const trampasSumadas = {};
            todasTrampasCapturaRastreros.forEach(item => {
                const key = `${item.id_trampa} (${item.ubicacion})`;
                if (!trampasSumadas[key]) {
                    trampasSumadas[key] = { id_trampa: item.id_trampa, trampa_nombre: item.trampa_nombre, ubicacion: item.ubicacion, cantidad_total: 0, total_capturas: 0 };
                }
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'TRAMPAS CON MAYOR CAPTURA - RASTREROS',
                            font: { size: 16, weight: 'bold' }
                        },
                        datalabels: { display: false },
                        tooltip: {
        ...
        // =============================================
        // 5. AREAS CAPTURA VOLADORES
        // =============================================
        (function() {
            const areasCapturasPorPlagaVoladores = <?= json_encode($areasCapturasPorPlagaVoladores ?? []) ?>;

            if (!areasCapturasPorPlagaVoladores || areasCapturasPorPlagaVoladores.length === 0) {
                const canvas = document.getElementById('areasCapturaVoladores');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    ctx.save();
                    ctx.textAlign = 'center';
            ...
            const ubicaciones = [...new Set(areasCapturasPorPlagaVoladores.map(item => item.ubicacion))];
            const todasPlagas = [...new Set(areasCapturasPorPlagaVoladores.map(item => item.tipo_plaga))];
            const datasets = [];
            const colores = generateDistinctColorPalette(todasPlagas.length);

            todasPlagas.forEach((plaga, index) => {
                const datosPorUbicacion = [];
                let tieneAlgunaCapturaPositiva = false;
                ubicaciones.forEach(ubicacion => {
                    const item = areasCapturasPorPlagaVoladores.find(item => item.ubicacion === ubicacion && item.tipo_plaga === plaga);
                    let valor = item ? parseInt(item.cantidad_total) : 0;
                    if (valor > 0) tieneAlgunaCapturaPositiva = true;
                    datosPorUbicacion.push(valor);
                });
                if (tieneAlgunaCapturaPositiva) {
                    datasets.push({
                        label: normalizarNombreJS(plaga),
                        data: datosPorUbicacion,
                        backgroundColor: colores[index].replace('0.8', '0.9'),
            ...
            new Chart(ctx, {
                type: 'bar',
                data: { labels: ubicaciones, datasets: datasets },
                options: {
                    responsive: true,
                    maintainAspectRatio: false,
                    layout: { padding: { left: 30, right: 40, top: 40, bottom: 30 } },
                    plugins: {
                        datalabels: { display: false },
                        legend: {
                            position: 'bottom',
                            align: 'center',
                            labels: {
                                boxWidth: 15, padding: 20, usePointStyle: true, pointStyle: 'circle',
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 13, weight: 'bold' }
                            },
                            title: {
                                display: true, text: 'Tipos de Plagas',
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 15, weight: 'bold' },
                                color: '#1f2937', padding: { top: 15, bottom: 15 }
                            }
                        },
                        title: {
                            display: true,
                            text: 'ÁREAS QUE PRESENTARON CAPTURAS - VOLADORES',
                            font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 18, weight: 'bold' },
                            color: '#1f2937', padding: { top: 15, bottom: 35 }
                        },
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    let label = context.dataset.label || '';
                                    if (label) label += ': ';
                                    if (context.parsed.y !== null) label += context.parsed.y + ' organismos';
                                    const ubicacion = ubicaciones[context.dataIndex];
                                    const plaga = context.dataset.label;
                                    const item = areasCapturasPorPlagaVoladores.find(item => item.ubicacion === ubicacion && item.tipo_plaga === plaga);
                                    if (item) return `${label} (${item.total} capturas)`;
                                    return label;
                                },
                                afterBody: function(tooltipItems) {
                                    const locationIndex = tooltipItems[0].dataIndex;
                                    let totalReal = 0;
                                    tooltipItems[0].chart.data.datasets.forEach(function(dataset) { totalReal += dataset.data[locationIndex] || 0; });
                                    return ['Total en esta ubicacion: ' + totalReal + ' organismos'];
                                }
                            },
                            backgroundColor: 'rgba(30,41,59,0.9)',
                    ...
                    scales: {
                        x: {
                            stacked: true,
                            title: {
                                display: true, text: 'Ubicación',
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 15, weight: 'bold' },
                                color: '#1f2937', padding: { top: 15, bottom: 5 }
                            },
                            grid: { display: false, drawBorder: false },
                            ticks: {
                                autoSkip: false, maxTicksLimit: 15, maxRotation: 90, minRotation: 60,
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 11, weight: 'bold' },
                        ...
                        y: {
                            ...
                            max: function(context) {
                                let maxStackedValue = 0;
                                if (!context.chart.data.datasets || context.chart.data.datasets.length === 0) return 100;
                                const numLocations = context.chart.data.labels ? context.chart.data.labels.length : 0;
                                for (let locationIndex = 0; locationIndex < numLocations; locationIndex++) {
                                    let stackSum = 0;
                                    for (let datasetIndex = 0; datasetIndex < context.chart.data.datasets.length; datasetIndex++) {
                                        stackSum += context.chart.data.datasets[datasetIndex].data[locationIndex] || 0;
                                    }
                                    if (stackSum > maxStackedValue) maxStackedValue = stackSum;
                                }
                                return Math.max(50, Math.ceil((maxStackedValue * 1.15) / 50) * 50);
                            },
                            title: {
                                display: true, text: 'Cantidad de Organismos',
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 15, weight: 'bold' },
                                color: '#1f2937', padding: { top: 5, bottom: 15 }
                            },
                            grid: { color: 'rgba(226,232,240,0.8)', borderDash: [4, 4], drawBorder: false },
                            ticks: {
                                precision: 0, stepSize: 50,
                                font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 12, weight: 'bold' },
        ...
        // =============================================
        // 6. AREAS CAPTURA RASTREROS
        // =============================================
        (function() {
            const areasCapturasPorPlagaRastreros = <?= json_encode($areasCapturasPorPlagaRastreros ?? []) ?>;

            if (!areasCapturasPorPlagaRastreros || areasCapturasPorPlagaRastreros.length === 0) {
                const canvas = document.getElementById('areasCapturaRastreros');
                if (canvas) {
                    const ctx = canvas.getContext('2d');
                    ctx.save();
                    ctx.textAlign = 'center';
            ...
            const ubicaciones = [...new Set(areasCapturasPorPlagaRastreros.map(item => item.ubicacion))];
            const todasPlagas = [...new Set(areasCapturasPorPlagaRastreros.map(item => item.tipo_plaga))];
            const datasets = [];
            const colores = generateDistinctColorPalette(todasPlagas.length);
            ...
            todasPlagas.forEach((plaga, index) => {
                ...
                let tieneAlgunaCapturaPositiva = false;
                ubicaciones.forEach(ubicacion => {
                    const item = areasCapturasPorPlagaRastreros.find(item => item.ubicacion === ubicacion && item.tipo_plaga === plaga);
                    let valor = item ? parseInt(item.cantidad_total) : 0;
                    if (valor > 0) tieneAlgunaCapturaPositiva = true;
                    datosPorUbicacion.push(valor);
                });
            ...
            new Chart(ctx, {
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        title: {
                            display: true,
                            text: 'ÁREAS QUE PRESENTARON CAPTURAS - RASTREROS',
                            font: { family: "'Segoe UI', 'Helvetica', 'Arial', sans-serif", size: 18, weight: 'bold' },
                            color: '#1f2937', padding: { top: 15, bottom: 35 }
                        },
                        tooltip: {
