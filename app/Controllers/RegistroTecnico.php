...
class RegistroTecnico extends BaseController
{
    ...
    public function index()
    {
        ...
        $planos = [];
        if ($sedeSeleccionada) {
            $planos = $this->planoModel->where('sede_id', $sedeSeleccionada)->where('habilitado', 1)->findAll();
    ...
    {
        try {
            $planos = $this->planoModel->where('sede_id', $sedeId)->where('habilitado', 1)->findAll();
    ...
    public function actualizar_id_trampa()
    {
        ...
        try {
            ...
            if ($dbId && is_numeric($dbId)) {
                $trampa = $this->trampaModel->find($dbId);
            }
