...
class Reports extends BaseController
{
    ...
    public function pdf_trampas($id = null)
    {
        ...
        if (!empty($trampas)) {
            ...
            // Consulta con validación de sede_id
            $query = $db->query("
                SELECT i.*
                FROM incidencias i
    ...
    public function reporte_visita()
    {
        ...
        if (!empty($todasTrampas)) {
            $trampaIds = array_column($todasTrampas, 'id');
            ...
            $query = $db->query("
                ...
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE DATE(i.fecha) = ?
                AND i.id_trampa IN (" . implode(',', $trampaIds) . ")
                AND t.sede_id = ?
                AND i.sede_id = t.sede_id
                ORDER BY i.fecha DESC
            ", [$fecha, $plano['sede_id']]);
            
            $incidenciasDia = $query->getResultArray();
        }
