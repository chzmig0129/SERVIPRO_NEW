...
class BlueprintsExcel extends BaseController
{
    ...
    private function generarTablaSemana($sheet, $trampas, $tiposInsectos, $fechaInicio, $plano, $sede, $tiposPlagaPorTrampa, $incluirHallazgos = false, $zonasHallazgos = [])
    {
        ...
        $fila = $this->generarTablaPorTipoTrampa($sheet, $trampas, $tiposPlagaPorTrampa['feromona_gorgojo'] ?? [], 'TRAMPA DE FEROMONA DE GORGOJO', $fila);
        
        // Generar tabla para Trampa de Feromona Palomilla
        $fila = $this->generarTablaPorTipoTrampa($sheet, $trampas, $tiposPlagaPorTrampa['feromona_palomilla'] ?? [], 'TRAMPA DE FEROMONA DE PALOMILLA', $fila);
        
        // Generar tabla para Trampa Atrayente Pulgas (solo cantidad)
        $fila = $this->generarTablaCantidadSimple($sheet, $trampas, 'TRAMPA ATRAYENTE PULGAS', $fila);
        
        // Generar tabla para Trampa Atrayente Chinches (solo cantidad)
        $fila = $this->generarTablaCantidadSimple($sheet, $trampas, 'TRAMPA ATRAYENTE CHINCHES', $fila);
        
        // Generar tabla para otras trampas (excluyendo Equipo Sónico y Globo de Terror)
    ...
    /**
     * Ordena trampas por números en el nombre (si contiene números)
     * Si no contiene números, mantiene el orden original
     */
    private function ordenarTrampasPorNumero($trampas) {
        usort($trampas, function($a, $b) {
            $nombreA = !empty($a['nombre']) ? $a['nombre'] : (!empty($a['id_trampa']) ? $a['id_trampa'] : 'T' . $a['id']);
            $nombreB = !empty($b['nombre']) ? $b['nombre'] : (!empty($b['id_trampa']) ? $b['id_trampa'] : 'T' . $b['id']);
            
            // Extraer números del nombre A
            preg_match_all('/\d+/', $nombreA, $matchesA);
            $numerosA = !empty($matchesA[0]) ? array_map('intval', $matchesA[0]) : [];
            
            // Extraer números del nombre B
            preg_match_all('/\d+/', $nombreB, $matchesB);
            $numerosB = !empty($matchesB[0]) ? array_map('intval', $matchesB[0]) : [];
    ...
    private function generarTablaPorTipoTrampa($sheet, $trampas, $tiposPlagaPermitidos, $nombreTipoTrampa, $filaInicio)
    {
        ...
        $palabrasClave = [
            ...
            'EQUIPO DE LUZ UV' => ['luz', 'uv'],
            'TRAMPA DE FEROMONA DE GORGOJO' => ['feromona', 'gorgojo'],
            'TRAMPA DE FEROMONA DE PALOMILLA' => ['feromona', 'palomilla']
        ];
        ...
        $trampasFiltradas = $this->ordenarTrampasPorNumero(array_values($trampasFiltradas));
        ...
        $sheet->setCellValue($columna . $fila, 'Total de organismos');
        $sheet->getStyle($columna . $fila)->getFont()->setBold(true);
        $sheet->getStyle($columna . $fila)->getFill()
            ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
            ->getStartColor()->setRGB('4472C4');
    ...
    private function generarTablaHallazgos($sheet, $zonasHallazgos, $tiposInsectos, $filaInicio)
    {
        ...
        // Filtrar tipos de insectos (excluir "Total de insectos" y "Total de organismos")
        $tiposSinTotal = array_filter($tiposInsectos, function($tipo) {
            return $tipo !== 'Total de insectos' && $tipo !== 'Total de organismos';
        });
    ...
    /**
     * Genera una tabla simple solo con cantidad de organismos (para Atrayente Chinches y Pulgas)
     */
    private function generarTablaCantidadSimple($sheet, $trampas, $nombreTipoTrampa, $filaInicio)
    {
        // Palabras clave para identificar cada tipo de trampa
        $palabrasClave = [
            'TRAMPA ATRAYENTE PULGAS' => ['atrayente', 'pulga'],
            'TRAMPA ATRAYENTE CHINCHES' => ['atrayente', 'chinche']
        ];
        
        $claves = $palabrasClave[$nombreTipoTrampa] ?? [];
        
        // Filtrar trampas por tipo usando palabras clave
        $trampasFiltradas = array_filter($trampas, function($trampa) use ($claves) {
            $tipoTrampaNormalizado = $this->normalizarTipoTrampa($trampa['tipo'] ?? '');
            return !empty($claves) && array_reduce($claves, function($carry, $clave) use ($tipoTrampaNormalizado) {
                return $carry && strpos($tipoTrampaNormalizado, $clave) !== false;
            }, true);
        });
        ...
        // Calcular número de columnas (ÁREA + EQUIPO + Cantidad de Organismos)
        $numColumnas = 3;
        $ultimaColumnaLetra = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($numColumnas);
        ...
        $sheet->setCellValue($columna . $fila, 'CANTIDAD DE ORGANISMOS');
        $sheet->getStyle($columna . $fila)->getFont()->setBold(true);
        $sheet->getStyle($columna . $fila)->getFill()
            ->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)
            ->getStartColor()->setRGB('4472C4');
    ...
    /**
     * Genera una tabla para otras trampas (excluyendo los tipos principales y Equipo Sónico/Globo de Terror)
     */
    private function generarTablaOtrasTrampas($sheet, $trampas, $tiposInsectos, $filaInicio)
    {
        ...
        $tiposConTablaPropia = ['edc_quimicas', 'edc_adhesivas', 'luz_uv', 'feromona_gorgojo', 'feromona_palomilla', 'atrayente_pulgas', 'atrayente_chinches'];
        // Tipos excluidos (tienen su propia vista de Estado de Trampas)
        $tiposExcluidos = ['equipo_sonico', 'globo_terror'];
        $palabrasClaveTodos = [
        ...
        $trampasFiltradas = array_filter($trampas, function($trampa) use ($tiposConTablaPropia, $tiposExcluidos, $palabrasClaveTodos) {
            $tipoTrampaNormalizado = $this->normalizarTipoTrampa($trampa['tipo'] ?? '');
            
            // Verificar si es de los tipos con tabla propia
            $tieneTablaPropia = false;
            foreach ($tiposConTablaPropia as $tipoPrincipal) {
                $claves = $palabrasClaveTodos[$tipoPrincipal] ?? [];
                if (!empty($claves)) {
                    $todasLasClavesPresentes = array_reduce($claves, function($carry, $clave) use ($tipoTrampaNormalizado) {
                        return $carry && strpos($tipoTrampaNormalizado, $clave) !== false;
                    }, true);
                    ...
                    if ($todasLasClavesPresentes) {
                        ...
                        if ($tipoPrincipal !== 'edc_quimicas' && $tipoPrincipal !== 'edc_adhesivas') {
                            $tieneTablaPropia = true;
                            break;
                        }
                    }
            ...
            $estaExcluido = false;
            foreach ($tiposExcluidos as $tipoExcluido) {
                $claves = $palabrasClaveTodos[$tipoExcluido] ?? [];
                if (!empty($claves)) {
                    $todasLasClavesPresentes = array_reduce($claves, function($carry, $clave) use ($tipoTrampaNormalizado) {
                        return $carry && strpos($tipoTrampaNormalizado, $clave) !== false;
                    }, true);
    ...
    public function procesarExcel($planoId = null)
    {
        ...
        try {
            ...
            $mapeoTiposInsectos = [
                ...
                // Organismos especiales para trampas específicas
                'Pulga' => ['tipo_plaga' => 'pulga', 'tipo_insecto' => 'Rastrero'],
                'Chinche' => ['tipo_plaga' => 'chinche', 'tipo_insecto' => 'Rastrero'],
                // Organismos para Trampa de Feromona Gorgojo
                'Sithopilus sp.' => ['tipo_plaga' => 'sithopilus', 'tipo_insecto' => 'Rastrero'],
                'Lasioderma sp.' => ['tipo_plaga' => 'lasioderma', 'tipo_insecto' => 'Rastrero'],
                'Tribolium sp.' => ['tipo_plaga' => 'tribolium', 'tipo_insecto' => 'Rastrero'],
                'Stegobium sp.' => ['tipo_plaga' => 'stegobium', 'tipo_insecto' => 'Rastrero'],
                'Rizopertha sp.' => ['tipo_plaga' => 'rizopertha', 'tipo_insecto' => 'Rastrero'],
                'Orizaephilus sp.' => ['tipo_plaga' => 'orizaephilus', 'tipo_insecto' => 'Rastrero'],
                'Trogoderma sp.' => ['tipo_plaga' => 'trogoderma', 'tipo_insecto' => 'Rastrero'],
                'Dermestes sp.' => ['tipo_plaga' => 'dermestes', 'tipo_insecto' => 'Rastrero'],
                'Tenebroides sp.' => ['tipo_plaga' => 'tenebroides', 'tipo_insecto' => 'Rastrero'],
                'Otro (gorgojo)' => ['tipo_plaga' => 'otro_gorgojo', 'tipo_insecto' => 'Rastrero'],
                // Organismos para Trampa de Feromona Palomilla
                'Plodia' => ['tipo_plaga' => 'plodia', 'tipo_insecto' => 'Volador'],
                'Ephestia' => ['tipo_plaga' => 'ephestia', 'tipo_insecto' => 'Volador'],
                'Sitotroga' => ['tipo_plaga' => 'sitotroga', 'tipo_insecto' => 'Volador'],
                'Otra (palomilla)' => ['tipo_plaga' => 'otra_palomilla', 'tipo_insecto' => 'Volador']
            ];
    ...
    private function procesarHojaExcel($sheet, $trampasIndex, $tiposInsectos, $mapeoTiposInsectos, $inspectorPorDefecto, $numeroSemana, $plano)
    {
        ...
        $titulosTablas = [
            'EDC QUÍMICAS', 'EDC ADHESIVAS', 'EQUIPO DE LUZ UV', 
            'TRAMPA DE FEROMONA DE GORGOJO', 'TRAMPA DE FEROMONA DE PALOMILLA',
            'TRAMPA ATRAYENTE PULGAS', 'TRAMPA ATRAYENTE CHINCHES',
            'OTRAS TRAMPAS', 'HALLAZGOS',
            'EDC QUIMICAS', 'EDC ADHESIVAS', 'EQUIPO DE LUZ UV'
        ];
        ...
        for ($fila = 1; $fila <= $ultimaFila; $fila++) {
            ...
            if ($esTituloTabla) {
                ...
                // Verificar si es tabla de chinches o pulgas (estructura especial con solo cantidad)
                $esTablaAtrayente = false;
                $tipoAtrayente = null; // 'chinche' o 'pulga'
                if (stripos($valorNormalizado, 'ATRAYENTE CHINCHES') !== false || stripos($valorNormalizado, 'ATRAYENTE CHINCHE') !== false) {
                    $esTablaAtrayente = true;
                    $tipoAtrayente = 'Chinche';
                } elseif (stripos($valorNormalizado, 'ATRAYENTE PULGAS') !== false || stripos($valorNormalizado, 'ATRAYENTE PULGA') !== false) {
                    $esTablaAtrayente = true;
                    $tipoAtrayente = 'Pulga';
                }
                        ...
                        if (strtoupper($valorArea) === 'ZONA') {
                            ...
                            while ($columnaIndex < $maxColumnas) {
                                ...
                                if ((stripos($valorColumna, 'Total') !== false && stripos($valorColumna, 'insectos') !== false) ||
                                    (stripos($valorColumna, 'Total') !== false && stripos($valorColumna, 'organismos') !== false)) {
                                    break;
                                }
                                ...
                                foreach ($tiposInsectos as $tipo) {
                                    if ($tipo === 'Total de insectos' || $tipo === 'Total de organismos') {
                                        continue;
                                    }
                ...
                // Procesar tablas de atrayente (chinches y pulgas) que tienen estructura especial
                if ($esTablaAtrayente && $tipoAtrayente) {
                    // Buscar los encabezados para estas tablas (ÁREA, EQUIPO, CANTIDAD DE ORGANISMOS)
                    $filaEncabezadosAtrayente = null;
                    for ($filaBuscar = $fila + 1; $filaBuscar <= min($fila + 5, $ultimaFila); $filaBuscar++) {
                        $valorArea = trim((string)$sheet->getCell('A' . $filaBuscar)->getValue());
                        $valorEquipo = trim((string)$sheet->getCell('B' . $filaBuscar)->getValue());
                        $valorCantidad = trim((string)$sheet->getCell('C' . $filaBuscar)->getValue());
                        
                        if ((strtoupper($valorArea) === 'ÁREA' || strtoupper($valorArea) === 'AREA') &&
                            strtoupper($valorEquipo) === 'EQUIPO' &&
                            (stripos($valorCantidad, 'CANTIDAD') !== false && stripos($valorCantidad, 'ORGANIS') !== false)) {
                            $filaEncabezadosAtrayente = $filaBuscar;
                            break;
                        }
                    }
                    
                    if ($filaEncabezadosAtrayente) {
                        // Buscar dónde termina esta tabla
                        $filaInicioDatos = $filaEncabezadosAtrayente + 1;
                        ...
                        for ($filaDatos = $filaInicioDatos; $filaDatos <= $filaFinDatos; $filaDatos++) {
                            // Obtener el ID del equipo (columna B)
                            ...
                            // Leer la cantidad de organismos (columna C)
                            $cell = $sheet->getCell('C' . $filaDatos);
                            $cantidad = null;
                            ...
                            // Si hay cantidad y el tipo de insecto está mapeado, crear la incidencia
                            if ($cantidad > 0 && isset($mapeoTiposInsectos[$tipoAtrayente])) {
                                $mapeo = $mapeoTiposInsectos[$tipoAtrayente];
