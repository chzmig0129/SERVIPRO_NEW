...
class Locations extends BaseController
{
    public function index(): string
    {
        ...
        try {
            ...
            // Voladores: misma query pero filtrada por tipo_insecto = 'Volador'
            $incidenciasPorTipoPlagaVoladoresQuery = "
                SELECT DATE_FORMAT(i.fecha, '%Y-%m') as mes, i.tipo_plaga, COUNT(*) as total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
                AND t.sede_id = {$sedeSeleccionada}
                AND i.tipo_insecto = 'Volador'
            ";
            if (!empty($condicionFecha)) {
                $incidenciasPorTipoPlagaVoladoresQuery .= $condicionFecha;
            }
            $incidenciasPorTipoPlagaVoladoresQuery .= "
                GROUP BY mes, i.tipo_plaga
                ORDER BY mes ASC
            ";
            $query = $db->query($incidenciasPorTipoPlagaVoladoresQuery);
            $data['incidenciasPorTipoPlagaVoladores'] = $query->getResultArray();

            // Rastreros: misma query pero filtrada por tipo_insecto = 'Rastrero'
            $incidenciasPorTipoPlagaRastrerosQuery = "
                SELECT DATE_FORMAT(i.fecha, '%Y-%m') as mes, i.tipo_plaga, COUNT(*) as total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
                AND t.sede_id = {$sedeSeleccionada}
                AND i.tipo_insecto = 'Rastrero'
            ";
            if (!empty($condicionFecha)) {
                $incidenciasPorTipoPlagaRastrerosQuery .= $condicionFecha;
            }
            $incidenciasPorTipoPlagaRastrerosQuery .= "
                GROUP BY mes, i.tipo_plaga
                ORDER BY mes ASC
            ";
            $query = $db->query($incidenciasPorTipoPlagaRastrerosQuery);
            $data['incidenciasPorTipoPlagaRastreros'] = $query->getResultArray();
            ...
            if ($sedeSeleccionada) {
                $planos = $planoModel->where('sede_id', $sedeSeleccionada)->where('habilitado', 1)->findAll();
            ...
            // Voladores: misma query pero filtrada por tipo_insecto = 'Volador'
            $todasTrampasCapturaVoladoresQuery = "
                SELECT t.id as id_trampa, t.nombre as trampa_nombre, t.ubicacion, i.tipo_plaga, COUNT(*) as total_capturas, SUM(i.cantidad_organismos) as cantidad_total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
            ...
            ";
            if (!empty($condicionFecha)) {
                $todasTrampasCapturaVoladoresQuery .= $condicionFecha;
            }
            $todasTrampasCapturaVoladoresQuery .= "
                GROUP BY t.id, t.nombre, t.ubicacion, i.tipo_plaga
                ORDER BY cantidad_total DESC, total_capturas DESC
            ";
            $query = $db->query($todasTrampasCapturaVoladoresQuery);
            $data['todasTrampasCapturaVoladores'] = $query->getResultArray();

            // Rastreros: misma query pero filtrada por tipo_insecto = 'Rastrero'
            $todasTrampasCapturaRastrerosQuery = "
                SELECT t.id as id_trampa, t.nombre as trampa_nombre, t.ubicacion, i.tipo_plaga, COUNT(*) as total_capturas, SUM(i.cantidad_organismos) as cantidad_total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
            ...
            ";
            if (!empty($condicionFecha)) {
                $todasTrampasCapturaRastrerosQuery .= $condicionFecha;
            }
            $todasTrampasCapturaRastrerosQuery .= "
                GROUP BY t.id, t.nombre, t.ubicacion, i.tipo_plaga
                ORDER BY cantidad_total DESC, total_capturas DESC
            ";
            $query = $db->query($todasTrampasCapturaRastrerosQuery);
            $data['todasTrampasCapturaRastreros'] = $query->getResultArray();
            ...
            // Voladores: misma query pero filtrada por tipo_insecto = 'Volador'
            $areasCapturasPorPlagaVoladoresQuery = "
                SELECT t.ubicacion, i.tipo_plaga, COUNT(*) as total, SUM(i.cantidad_organismos) as cantidad_total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
            ...
            ";
            if (!empty($condicionFecha)) {
                $areasCapturasPorPlagaVoladoresQuery .= $condicionFecha;
            }
            $areasCapturasPorPlagaVoladoresQuery .= "
                GROUP BY t.ubicacion, i.tipo_plaga
                ORDER BY cantidad_total DESC, total DESC
            ";
            $query = $db->query($areasCapturasPorPlagaVoladoresQuery);
            $data['areasCapturasPorPlagaVoladores'] = $query->getResultArray();

            // Rastreros: misma query pero filtrada por tipo_insecto = 'Rastrero'
            $areasCapturasPorPlagaRastrerosQuery = "
                SELECT t.ubicacion, i.tipo_plaga, COUNT(*) as total, SUM(i.cantidad_organismos) as cantidad_total
                FROM incidencias i
                INNER JOIN trampas t ON i.id_trampa = t.id
                WHERE i.sede_id = {$sedeSeleccionada}
            ...
            ";
            if (!empty($condicionFecha)) {
                $areasCapturasPorPlagaRastrerosQuery .= $condicionFecha;
            }
            $areasCapturasPorPlagaRastrerosQuery .= "
                GROUP BY t.ubicacion, i.tipo_plaga
                ORDER BY cantidad_total DESC, total DESC
            ";
            $query = $db->query($areasCapturasPorPlagaRastrerosQuery);
            $data['areasCapturasPorPlagaRastreros'] = $query->getResultArray();
